...
$query="SELECT u.url, u.url_page_name, u.url_tag FROM campaigns c, urls u WHERE c.c_code=u.c_code AND c.c_name='$ln{campaignBox}' order by u.url_tag";
  ...
  $query1="SELECT clientAddress, localTime, httpRequest, concat(year(prodate), '/',month(prodate), '/',dayofmonth(prodate)), s_id FROM log_stats order by prodate";
...
$initdate = 0; 
  foreach $key_url(sort hshvalue1(keys(%httpHasho))){
    if($initdate eq 0) {
	$pdate = ($httpHasho{$key_url})->visit_date();
	$initdate = 1;
    }
if( $ln{reportBox} eq 'daily') {
    if($pdate ne ($httpHasho{$key_url})->visit_date()){
#	
#
#
#################################start_of_table#####################################
