...
use Time::Local;
#----------------<getting data from either post or get>--------------#
$method = $ENV{'REQUEST_METHOD'};

print "Data Passed Via $method method<p>";

if ($method eq "GET") { 
  $datafrom = $ENV{'QUERY_STRING'}; } 
elsif ($method eq "POST") { 
  read(STDIN,$datafrom,$ENV{'CONTENT_LENGTH'});} 
else { 
  $datafrom ="";}
$datafrom =~ s/\+/ /g;

  # split form data and store in hash
  @details = split (/&/, $datafrom);
...
#----------------<end of getting data from either post or get>--------------#
...
<META HTTP-EQUIV="refresh" content="1;URL=../tracklivev1/trendstats.php?c_id=$ln{c_id}"> 
</HEAD> 
<body>
